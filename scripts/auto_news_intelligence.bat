...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$Log = Join-Path $LogDir ('auto_news_intelligence_' + $Stamp + '.log');" ^
  ...
  "Write-Log 'START' 'auto_news_intelligence workflow';" ^
  ...
  "if (-not (Invoke-RetryPython 'scripts\investment_report.py' 'investment_report.py' @('--save', '--window-hours', '24'))) { Write-Log 'ERROR' 'investment_report.py failed after 3 retries'; exit 1 }" ^
